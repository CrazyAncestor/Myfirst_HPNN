...
class Homogeneous_Poisson_NN(tf.keras.models.Model):
    def __init__(self, ndims, data_format = 'channels_first', final_convolutions_config = None, pre_bottleneck_convolutions_config = None, bottleneck_upsampling = 'deconv', bottleneck_config = None, use_batchnorm = False, input_normalization = None, output_scaling = None):
        ...
        if bottleneck_upsampling == 'deconv':
            fields_in_bottleneck_cfg = ['downsampling_factors', 'upsampling_factors', 'conv_kernel_sizes', 'deconv_kernel_sizes', 'n_convs'] + (['conv_downsampling_kernel_sizes'] if ('conv_downsampling_kernel_sizes' in bottleneck_config.keys()) else [])#bottleneck_config OR bottleneck_deconv_config
            fields_in_bottleneck_args = ['downsampling_factor', 'upsampling_factor', 'conv_kernel_size', 'deconv_kernel_size', 'n_convs'] + (['conv_downsampling_kernel_size'] if ('conv_downsampling_kernel_sizes' in bottleneck_config.keys()) else [])#bottleneck_config OR bottleneck_deconv_config
            ...
            fields_in_bottleneck_cfg = ['downsampling_factors', 'upsampling_factors', 'conv_kernel_sizes', 'n_convs'] + (['conv_downsampling_kernel_sizes'] if ('conv_downsampling_kernel_sizes' in bottleneck_config.keys()) else [])#bottleneck_config OR bottleneck_deconv_config
            fields_in_bottleneck_args = ['downsampling_factor', 'upsampling_factor', 'conv_kernel_size', 'n_convs'] + (['conv_downsampling_kernel_size'] if ('conv_downsampling_kernel_sizes' in bottleneck_config.keys()) else [])#bottleneck_config OR bottleneck_deconv_config
